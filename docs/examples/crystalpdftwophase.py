...
def makeRecipe(niciffile, siciffile, datname):
    ...
    recipe.add_penalty("a_ni", lb=3.527, ub=3.527, scaled=True)
    # Now we do the same with the delta2 and Biso parameters (remember that
    # Biso = 8*pi**2*Uiso)
    recipe.add_penalty("delta2_ni", lb=2.22, ub=2.22, scaled=True)
    recipe.add_penalty("Biso_0_ni", lb=0.454, ub=0.454, scaled=True)
    #
    ...
    # as well.
    recipe.add_penalty("a_si", lb=5.430, ub=5.430, scaled=True)
    recipe.add_penalty("delta2_si", lb=3.54, ub=3.54, scaled=True)
    recipe.add_penalty("Biso_0_si", lb=0.645, ub=0.645, scaled=True)
