...
from that example, and simultaneoulsy refines a model to two data sets
...
def makeModel(strufile, datname1, datname2):
    ...
    We will create two Contributions that use the IntensityCalculator from
    npintensitycalculator.py and associate each of these with a Profile, and
    use this to define a FitModel.

    Both simulated data sets come from the same structure. We're going to make
    two contributions that are identical, except for the profile that is held
    in each. We're going to assure that the structures are identical by using
    the same StructureParSet (which is generated by the IntensityCalculator
    when we load the structure) in both calculators.

    """

    ## The Profiles
    # Create two Profiles for the two calculators.
    profile1 = Profile()
    profile2 = Profile()
    ...
    # conflict, since the name is only meaningful within the Contribution that
    # holds the Calculator.  Load the structure into one and set the second
    # Calculator's 'structure' attribute equal to that of the first. This will
    ...
    ## The Contributions
    # Create the Contributions. Note that the "q" in each of the Contributions
    # refers to that Contributions' Profile.
    ...
    # Now we're ready to define the fitting equation for each Contribution.
    ...
    # same names.  Contributions are isolated namespaces than only share
    ...
    # Make a FitModel and associate the Contributions.
    model = FitModel()
    ...
    # background variables that we just defined in the contribution. We have to
    # do this separately for each contribution.
    ...
    # We can also refine structural parameters. We only have to do this
    # once, since each calculator holds the same 'structure' ParameterSet.
